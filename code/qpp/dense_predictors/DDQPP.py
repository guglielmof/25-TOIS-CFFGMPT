...
class DDQPP(PDQPP):
    ...
    def _local_predict(self, query: pd.Series) -> float:
        ...
        cov = np.cov(top_k_docs_embs, rowvar=False)

        local_pred = np.array([_compute_local_predictor(query.representation, top_k_docs_embs, npr.multivariate_normal(mean=top_k_docs_embs.mean(axis=0), cov=cov)) for d in pseudo_relevant_embs])
