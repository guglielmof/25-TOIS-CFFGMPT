...
class RDQPP(PDQPP):
    ...
    def _local_predict(self, query: pd.Series) -> float:
        ...
        local_pred = np.array([_compute_local_predictor(query.representation, top_k_docs_embs, npr.normal(size=top_k_docs_embs.shape[1])) for d in pseudo_relevant_embs])
